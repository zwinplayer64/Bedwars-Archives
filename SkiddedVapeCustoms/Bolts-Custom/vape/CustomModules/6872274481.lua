...
task.spawn(function()
	...
	local OceanAmbience = {["Enabled"] = false}
	OceanAmbience = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
    	["Name"] = "OceanAmbience",
    	["HoverText"] = "module fixed by @Spring67 <3",
    	["Function"] = function(callback)
        	if callback then
				game.Lighting.Ambient = Color3.fromRGB(140, 0, 255)
            	game.Lighting.OutdoorAmbient = Color3.fromRGB(140, 0, 255)
        	else
	...
	local BloodAmbience = {["Enabled"] = false}
	BloodAmbience = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
    	["Name"] = "BloodAmbience",
    	["HoverText"] = "module fixed by @Spring67 <3",
    	["Function"] = function(callback)
        	if callback then
...
local funnyAura = {Enabled = false}
 ...
 PlayerTP = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
            ["Name"] = "Player Death TP",
            ["Function"] = function(callback)
                if callback then
                       for i, v in pairs(game:GetService("Players"):GetPlayers()) do
if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and v.Team ~= game.Players.LocalPlayer.Team and v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("Humanoid").Health ~= 0 then
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Character.HumanoidRootPart.CFrame + Vector3.new(0,5,6)
end
end
                else
                    wait(0.1)
...
repeat wait(0.2) 
    ...
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = otherbed.CFrame + Vector3.new(0,5,6)
    end    
else
        otherbed = nil
        break;
